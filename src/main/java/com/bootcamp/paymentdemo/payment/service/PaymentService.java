...
import com.bootcamp.paymentdemo.order.service.OrderService; // [중요]
...
public class PaymentService {
    ...
    private final OrderService orderService; // 모든 뒷수습은 얘가 담당

    // PointService, MembershipService 직접 주입 불필요 (OrderService가 함)
    ...
    public PaymentConfirmResponse confirmPayment(String dbPaymentId, String impUid) {
        ...
        try {
            ...
            if (expectedPayAmount.compareTo(actualPayAmount) != 0) {
                log.error("금액 불일치! DB 데이터 : {}, PortOne 데이터 : {}", expectedPayAmount, actualPayAmount);
