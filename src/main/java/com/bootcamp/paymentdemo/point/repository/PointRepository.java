...
public interface PointRepository extends JpaRepository<PointTransaction, Long> {
    ...
    List<PointTransaction> findByUser_UserIdAndTypeAndRemainingAmountGreaterThanAndExpiresAtAfterOrderByExpiresAtAsc(Long userId, PointType pointType, int i, LocalDate now);

    List<PointTransaction> findByTypeAndRemainingAmountGreaterThanAndExpiresAtBefore(PointType pointType, int i, LocalDate now);
}
